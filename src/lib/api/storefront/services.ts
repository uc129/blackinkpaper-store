...
export const storefrontProductService = {
  ...
  getById: (id: number) =>
    apiClient.get<ProductResponseDto>(`/api/products/${id}`),
  getBySlug: (slug: string) =>
    apiClient.get<ProductResponseDto>(
...
export const authService = {
  ...
  login: (payload: LoginRequest) =>
    apiClient.post<AuthResponse>("/api/Accounts/login", payload),
  logout: () =>
    apiClient.post<void>("/api/Accounts/logout", undefined, { auth: true }),
  profile: () =>
...
export const cartService = {
  ...
  updateQuantity: (
    ...
    apiClient.put<CartResponseDto>(`/api/cart/items/${cartItemId}`, payload, {
      auth: true,
    }),
  removeItem: (cartItemId: number) =>
    apiClient.delete<CartResponseDto>(`/api/cart/items/${cartItemId}`, {
      auth: true,
    }),
};
...
export const shippingAddressService = {
  list: () =>
    apiClient.get<ShippingAddressDto[]>("/api/shipping-addresses", {
      auth: true,
    }),
  create: (payload: CreateShippingAddressRequest) =>
    apiClient.post<ShippingAddressDto>("/api/shipping-addresses", payload, {
      auth: true,
    }),
  update: (id: number, payload: UpdateShippingAddressRequest) =>
    apiClient.put<ShippingAddressDto>(
...
export const checkoutService = {
  preview: (payload: CheckoutPreviewRequest) =>
    apiClient.post<CheckoutPreviewDto>("/api/checkout/preview", payload, {
      auth: true,
    }),
  createPaymentSession: (payload: CheckoutPreviewRequest) =>
    apiClient.post<PaymentSessionDto>(
      "/api/checkout/payment-session",
      payload,
      { auth: true },
    ),
  verifyPayment: (payload: VerifyRazorpayPaymentRequest) =>
    apiClient.post<OrderDto>("/api/checkout/verify-payment", payload, {
      auth: true,
    }),
  placeOrder: (payload: CheckoutPreviewRequest) =>
    apiClient.post<PlaceOrderResponseDto>(
  ...
  cancelOrder: (orderId: number) =>
    apiClient.post<void>(`/api/checkout/orders/${orderId}/cancel`, undefined, {
      auth: true,
    }),
};
