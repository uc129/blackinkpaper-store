  ...
  if (authStatus !== "authenticated") {
    ...
    cartContent = (
      <ContainerSimple className="max-w-4xl mx-auto py-8">
        <h1 className="font-display text-4xl font-bold mb-8 text-[var(--ink)]">
          Your Shopping Cart
        </h1>
        ...
        <div className="flex flex-col gap-8">
          {cartItems.map((item) => {
            ...
            const invalidQuantity = isCartItemQuantityInvalid(item);
              ...
              >
                <div className="flex-1">
                  ...
                  <div className="mt-2 flex flex-wrap gap-2 text-xs">
                    {item.isOriginal && (
                      <span className="rounded-full border border-[var(--border)] px-3 py-1 text-[var(--ink)]">
                        Original · 1 of 1
                      </span>
                    )}
                    {(unavailable || invalidQuantity) && (
                      <span className="rounded-full border border-[var(--danger)] px-3 py-1 text-[var(--danger)]">
                        {unavailable
                          ? "No longer available"
                          : `Only ${quantityLimit} available`}
                      </span>
                    )}
                  </div>
                  ...
                  <div className="mt-3 inline-block w-full max-w-sm store-surface p-4">
                    ...
                    {item.selectedVariants.map((variant) => (
                      <div
                        key={`${variant.productVariantId}-${variant.productVariantOptionId}`}
                        className="flex justify-between text-sm py-1 border-t border-[var(--border)]"
                      >
                ...
                <div className="flex flex-col items-end justify-between gap-4">
                  <div className="flex items-center gap-6">
                    <div className="flex flex-col items-end">
                      <span className="text-xs text-[var(--muted)] font-medium mb-1">
                        Quantity
                      </span>
                      <input
                        type="number"
                        ...
                        disabled={unavailable || item.isOriginal}
                        value={item.quantity}
            ...
            {hasAvailabilityIssues ? (
              <>
                <p className="text-sm text-[var(--danger)]">
                  Remove unavailable items or correct their quantities before
                  checkout.
                </p>
                <Button
                  disabled
