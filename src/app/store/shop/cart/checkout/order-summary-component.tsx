...
export function OrderSummary({ preview }: { preview: CheckoutPreviewDto | null }) {
  ...
  return (
    <div className="store-surface p-6 sticky top-8">
      <h2 className="font-display text-2xl font-bold mb-4 text-[var(--ink)]">Order Summary</h2>
              ...
              <div key={item.cartItemId} className="flex justify-between text-sm">
                <div className="flex-1">
                  <p className="font-semibold">
                    {item.name} <span className="text-[var(--muted)]">x{item.quantity}</span>
                  </p>
      ...
      <div className="border-t border-[var(--border)] pt-4 space-y-2">
        <div className="flex justify-between text-[var(--ink-soft)]">
          <span>Subtotal</span>
          <span>
            {currencyCode} {(preview?.subtotal ?? adjustedCartSubtotal).toFixed(2)}
          </span>
        </div>
        <div className="flex justify-between text-[var(--ink-soft)]">
          <span>Shipping</span>
          <span>{preview ? `${currencyCode} ${preview.shippingAmount.toFixed(2)}` : "Select address"}</span>
        </div>
        <div className="flex justify-between text-[var(--ink-soft)]">
          <span>Tax</span>
          <span>{preview ? `${currencyCode} ${preview.taxAmount.toFixed(2)}` : "Select address"}</span>
        </div>
        <div className="flex justify-between text-xl font-bold pt-2 border-t border-[var(--border)] mt-2">
          <span>Total</span>
          <span>
            {currencyCode} {(preview?.totalAmount ?? adjustedCartSubtotal).toFixed(2)}
          </span>
        </div>
      </div>
    </div>
